...
vi.mock('@/components', () => ({
  ...
  ModalActions: (props: {
    ...
    <div>
      <button onClick={props.onSuccess} disabled={props.disabled}>Save</button>
      <button onClick={props.onClose} disabled={props.disabled}>Close</button>
    </div>
  ),
  Access: ({ children }: { children: React.ReactNode }) => <>{children}</>,
  DialogActions: (props: React.HTMLAttributes<HTMLDivElement>) => <div {...props} />,
...
describe('ContractNote alternative test', () => {
  ...
  test('renders fields and disables note input if useAccess returns true', () => {
    const useAccessMock = vi.spyOn(accessHook, 'default').mockReturnValue(() => true);
  ...
  test('calls onSuccess with updated note', () => {
    const useAccessMock = vi.spyOn(accessHook, 'default').mockReturnValue(() => false);
    render(<ContractNote {...baseProps} />);
    const noteInput = screen.getByLabelText('common.note') as HTMLInputElement;
