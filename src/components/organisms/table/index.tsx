        ...
        {columns
          ...
          .map((column, index) => (
            ...
            >
              ...
              {column.id &&
                column.id === sortField &&
                (sortDirection === 'ASC' ? IconUp : IconDown)}
            </th>
          ))}
      </tr>
    </thead>
          ...
          >
            {columns
              ...
              .map((column: Column<T>, index) => {
                const key = column.id;
                const value =
                  key && !isEmptyObject(row) ? accessObject(row, key) : '';
  ...
  return (
    <>
      <div
        ...
        style={{ marginTop }}
        data-disabled-empty-data={emptyData}
      >
        <table
      ...
      {card &&
        Array.isArray(data) &&
        data.map((item) => card(item, styles.card))}
    </>
  );
};
