...
const PostInventory = () => {
  ...
  useEffect(() => {
    setTotalUnits(materialUnits.reduce((sum, inv) => sum + inv.units, 0));
    setTotalValue(merchandiseValue.reduce((sum, inv) => sum + inv.value, 0));
  }, [materialUnits, merchandiseValue]);

  const calculateUnitsPercentage = (units: number) =>
    totalUnits === 0 ? 0 : ((units / totalUnits) * 100).toFixed(2);
  ...
  return (
    <Grid container spacing={3}>
      ...
      <Grid size={{ xs: 12, md: 6 }}>
        ...
        >
          <Typography variant="h5" gutterBottom>
            {t('adminPostMovement.outboundChartTitle')}
          </Typography>
          <Table
            size="small"
            rows={materialUnits}
      ...
      <Grid size={{ xs: 12, md: 6 }}>
        ...
        >
          <Typography variant="h5" gutterBottom>
            {t('adminPostMovement.inboundChartTitle')}
          </Typography>
          <Table
            size="small"
