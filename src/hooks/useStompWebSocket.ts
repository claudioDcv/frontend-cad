...
export const useStompWebSocket = (config: StompConfig): StompConnection => {
  ...
  const [lastNotification, setLastNotification] = useState<StompMessage | null>(
    null
  );
  ...
  const validateJWT = useCallback(
    (token: string): boolean => {
      ...
      try {
        ...
        addDebugMessage(
          `Token parseado - Usuario: ${payload.sub || 'N/A'}, Nombre: ${
            payload.nombre || 'N/A'
          } ${payload.apellido || 'N/A'}`
        );
        return true;
      } catch {
        addDebugMessage(
          'No se pudo parsear la información del token, pero intentando conectar...',
          'error'
        );
  ...
  const subscribeToTopics = useCallback(() => {
    ...
    if (!client) {
      addDebugMessage(
        '❌ No hay cliente STOMP disponible para subscripción',
        'error'
      );
      return;
    }
    ...
    if (!client.connected) {
      addDebugMessage(
        '❌ Cliente STOMP no está conectado para subscripción',
        'error'
      );
      return;
    }
    ...
    try {
      ...
      client.subscribe('/topic/notifications', (message) => {
        try {
          ...
          addDebugMessage(
            `📢 NOTIFICACIÓN recibida en /topic/notifications: ${JSON.stringify(
              notification,
              null,
              2
            )}`,
      ...
      addDebugMessage(
        '✅ Suscripción a /topic/notifications exitosa',
        'success'
      );
      ...
      client.subscribe('/user/queue/pong', (message) => {
        try {
          ...
          addDebugMessage(
            `🏓 PONG recibido: ${JSON.stringify(pong, null, 2)}`,
            'success'
          );
        } catch (error) {
      ...
      addDebugMessage(
        '📡 Suscrito a /topic/notifications y /user/queue/pong - Auto-subscripción completa!',
        'success'
      );
    } catch (error) {
  ...
  const connect = useCallback(() => {
    ...
    setError(null);
    addDebugMessage(
      `🔄 Intentando conectar con token: ${token.substring(0, 20)}...`
    );
    ...
    try {
      ...
      const debugObj = {
        debug: config.debug
          ? // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (str: any) => console.log('STOMP Debug:', str)
          : undefined,
      };
      ...
      client.onConnect = (frame) => {
        addDebugMessage(
          '✅ Conectado exitosamente al servidor STOMP',
          'success'
        );
      ...
      client.onWebSocketClose = (event) => {
        addDebugMessage(
          `WebSocket cerrado: ${event.code} - ${event.reason}`,
          'info'
        );
        setIsConnected(false);
        setIsConnecting(false);
        ...
        if (
          ...
          setTimeout(() => {
            addDebugMessage(
              `🔄 Reintentando conexión (intento ${reconnectAttempts + 1})`,
              'info'
            );
            connect();
          }, 5000);
  ...
  const sendNotification = useCallback(
    (notification: StompMessage) => {
      ...
      try {
        ...
        addDebugMessage(
          `📤 Notificación enviada: "${notificationWithDefaults.payload}"`,
          'success'
        );
      } catch (error) {
  ...
  useEffect(() => {
    if (
      ...
      if (jwtParts.length === 3) {
        ...
        addDebugMessage(
          'Token no tiene formato JWT válido, no auto-conectando',
          'error'
        );
      }
